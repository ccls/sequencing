...
[10,100,0,1].each do |num_descriptions|
[10,100,0,1].each do |num_alignments|
[10,100,1].each do |max_target_seqs|
#[4,8,12].each do |word_size|
[false,true].each do |ungapped|
[0,5].each do |outfmt|
%w( 1e-1 1e-10 1e-50 1e-100 ).each do |evalue|
...
#	Warning: Number of descriptions overridden to 10, number of alignments overridden to 10.
			...
			time_parts = time_output.split
	
			c << [iteration,fasta,evalue,num_threads,strand,num_descriptions,num_alignments,
