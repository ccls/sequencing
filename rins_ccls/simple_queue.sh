...
#	Changing the default timeout of 0 to 1000 ms or something will allow the command
...
#
#sqlite="sqlite3 -cmd '.timeout 1000' $database_file_name "
#	using this variable doesn't work?
#sqlite3 -cmd '.timeout 1000' /Users/jakewendt/simple_queue.db
...
#Use -help for a list of options.
#
#sh-3.2$ sqlite="sqlite3 -cmd '.timeout 1000' $HOME/simple_queue.db "
#sh-3.2$ $sqlite
...
pop(){
	r=`sqlite3 -cmd '.timeout 1000' -line $database_file_name "select * from queue order by id asc limit 1"`
...
#	the "^\s*" doesn't work on ec0000?? different grep probably
	...
	if [ "x$id" != "x"  ] ; then
		...
		echo $command
		sqlite3 -cmd '.timeout 1000' $database_file_name "delete from queue where id = $id"
...
if [ ! -f $database_file_name ] ; then
	sqlite3 -cmd '.timeout 1000' $database_file_name 'create table queue(id integer primary key autoincrement, command text)'
...
#		shift; $sqlite "insert into queue(command) values('$*')";;
		shift; sqlite3 -cmd '.timeout 1000' $database_file_name "insert into queue(command) values('$*')";;
	size | count | length )
#		$sqlite "select count(*) from queue" ;;
		sqlite3 -cmd '.timeout 1000' $database_file_name "select count(*) from queue" ;;
	* )
#		$sqlite "select * from queue"
		sqlite3 -cmd '.timeout 1000' $database_file_name "select * from queue"
esac
