...
ARGV.each do |filename|
	...
	command = "wc -l #{filename} | awk '{print $1/4}'"
	...
	#	The "sort | uniq -d" is what ensures that the results are in pairs

	command = "awk '( (NR-1)%4 == 0 ){print}' #{filename} | awk -F/ '{print $1}' | sort | uniq -d | sed 's/^@//'"
