...
my $VERSION = "Trinity-r2012-10-05"; 
...
unless ($java_version =~ /(java|openjdk) version \"1\.[6]\./) {
...
main: {
    ...
    if ($run_with_collectl){
        ...
        system($cmd);
        system("sync; sleep 1"); #try to make sure that collectl has started
        $collectl_pid = `pidof -x collectl`;
        print "COLLECTL PID = $collectl_pid\n";
    }
