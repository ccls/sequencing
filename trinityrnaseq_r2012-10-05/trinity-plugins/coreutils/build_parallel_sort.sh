...
if [ $MAJOR -gt 8 -o $MAJOR -eq 8 -a $MINOR -ge 6 ]; then
  echo "sort is recent enough, bailing out"
  mkdir -p ${DIR}/bin
  cd ${DIR}/bin
  SORTPATH=`which sort`
  ln -s ${SORTPATH}
...
if [ ! -d ${INSTALLDIR} ] ; then
    echo "sort installation went terribly wrong"
    exit 127
fi
cd ${INSTALLDIR}
./configure
...
if [ ! -f ./sort ]; then
  echo "#!/bin/bash
  echo \"this script will remove the first argument which has to be the --parallel=NCPU\"
  shift
  sort \$*
  ">sort
  chmod +x ./sort
