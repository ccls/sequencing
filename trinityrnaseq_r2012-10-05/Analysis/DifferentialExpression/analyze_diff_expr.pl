...
sub parse_result_files_find_diffExp {
    ...
    my $result_summary_file = "all_diff_expression_results.txt";
    unless (-s $result_summary_file) {
        die "Error, cannot find edgeR summary file: $result_summary_file";
    }
    
    open (my $fh, $result_summary_file) or die "Error, cannot open file $result_summary_file";
    my $header = <$fh>;
    while (<$fh>) {
        if (/^\#/) { next; }
        chomp;
        my ($sampleA, $sampleB, $gene, $log_conc, $log_fold_change, $p_value, $fdr) = split(/\s+/);
