...
int main(int argc,char** argv)
{
    ...
    if (PAIRED_READS_MODE) {
        ...
        if (Exists(scaffolds_finished_file) && Exists("iworm_scaffolds.txt")) {
            ...
            cmdstr << exec 
                   ...
                   << " --seqType fa "
                   << " --single " << readString
                   << " --sort_buffer_size " << sortBufferSizeString
                   << " -o iworm_bowtie "
                   << " --retain_SAM_files ";
            ...
            */
            ...
            cmdstr << "bowtie-build target.fa target";
            cerr << "CMD: " << cmdstr.str() << endl;
            Execute(cmdstr.str().c_str());
            ...
            cmdstr << "bowtie -a -m 20 --best --strata --threads " << nCPU 
                   ...
                   << " -f target " << readString << " > bowtie.out";
    ...
    /*  this section of code is unnecessary  - bhaas

    if (tmpSeq.isize() > 0) {
        
        // capture last one

        cerr << "Error parsing components output file." << endl;
        exit(1);
