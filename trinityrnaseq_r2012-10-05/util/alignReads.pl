...
#
#  --aligner <string>           BLAT, bowtie, bwa, gsnap, or tophat  
#
# Optional:
#
...
unless ($aligner && $aligner =~ /^BLAT|bowtie|bwa|gsnap|tophat$/) {
...
main: {
    ...
    unless (-s "target.fa") {
        ...
        if ($aligner eq "bowtie" || $aligner eq "tophat") {
            ...
            my @bowtie_build_files = <$target_db.*.ebwt>;
            ...
            unless (@bowtie_build_files) {
                ...
                my $start_index_file = "$target_db.ebwt.start";
                ...
                my $cmd = "bowtie-build $target_db $target_db";
                &process_cmd($cmd);
            ...
            @bowtie_build_files = <$target_db.*.ebwt>;
    ...
    if ($aligner eq "tophat") {
        ...
        foreach my $fa_info (@entries) {
            ...
            eval {
                ...
                if ($aligner eq "BLAT") {
                    ...
                    &run_BLAT_alignment_pipeline($target_fa, $trans_file);
                    ...
                    &run_bowtie_alignment_pipeline($target_fa, $trans_file);
        ...
        if ($SS_lib_type) {
            ...
            $cmd = "touch $outfile_basename.coordSorted.sam.finished";
            &process_cmd($cmd) unless (-e "touch $outfile_basename.coordSorted.sam.finished");
        ...
        if ($SS_lib_type) {
            ...
            foreach my $sam_file ("$outfile_basename.coordSorted.sam.+.sam", "$outfile_basename.coordSorted.sam.-.sam",
                ...
                if (-s $sam_file) {
                    ...
                    if ( (! $single_file) && $bam_file =~ /nameSorted/) {
                        ...
                        unless ($NO_BAM) {
                            ...
                            &process_cmd($cmd) if ( (! -s "$sam_file.PropMapPairsForRSEM.pre.bam") && -s "$sam_file.PropMapPairsForRSEM.sam" 
                                                    && (! -s "$sam_file.PropMapPairsForRSEM.bam"));
            ...
            if ( (! $single_file) && (! $NO_BAM) ) {
                ...
                &process_cmd($cmd) if ( (! -s "$outfile_basename.nameSorted.PropMapPairsForRSEM.pre.bam") && -s "$outfile_basename.nameSorted.PropMapPairsForRSEM.sam");
...
sub make_RSEM_bam {
    ...
    my $cmd = "$RSEM_dir/convert-sam-for-rsem $pre_bam_file $out_bam_prefix -T . ";
    &process_cmd($cmd);
...
sub run_BLAT_alignment_pipeline {
    my ($target_fa, $trans_file) = @_;
        
    ## Prep sequences>
    if ($seqType eq "fq") {
        my $cmd = "$util_dir/fastQ_to_tab.pl -I $trans_file > $target_fa.tab";
        &process_cmd($cmd) unless (-s "$target_fa.tab");
    }
    else {
        my $cmd = "$util_dir/fasta_to_tab.pl  $trans_file > $target_fa.tab";
        &process_cmd($cmd) unless (-s "$target_fa.tab");
    }
    
    my $cmd = "sort -T . -S $sort_buffer_size -k 1,1 -k 3,3 $target_fa.tab > $target_fa.sorted.tab";
    ...
    ## sort by name
    $cmd = "sort -T . -S $sort_buffer_size -k 1,1 -k 3,3 $target_fa.psl.sam > $target_fa.psl.nameSorted.sam";
...
sub run_bowtie_alignment_pipeline {
    ...
    my @bowtie_opts = @ARGV;
    if (! grep { $_ eq "-a" } @bowtie_opts) {
        push (@bowtie_opts, "-k", $num_hits);  ## don't specifiy the number of hits if using the -a (all) option.
    }

    my $cmd = "bowtie @bowtie_opts -q -S --sam-nohead $format target $target_fa > $target_fa.pre.sam";
    &process_cmd($cmd) unless (-e "$target_fa.pre.sam.finished");
    $cmd = "touch $target_fa.pre.sam.finished";
    &process_cmd($cmd) unless (-e "$target_fa.pre.sam.finished");
        
    ## remove unaligned reads
    $cmd = "$util_dir/SAM_filter_out_unmapped_reads.pl $target_fa.pre.sam > $target_fa.sam";
    &process_cmd($cmd) unless (-e "$target_fa.sam.finished");
    $cmd = "touch $target_fa.sam.finished";
    &process_cmd($cmd) unless (-e "$target_fa.sam.finished");
    unless ($retain_intermediate_files_flag) {
        &purge_files("$target_fa.pre.sam");
    }

    # name-sort 
    $cmd = "sort -T . -S $sort_buffer_size -k 1,1 -k 3,3 $target_fa.sam > $target_fa.nameSorted.sam";
    &process_cmd($cmd) unless (-e "$target_fa.nameSorted.sam.finished");
    $cmd = "touch $target_fa.nameSorted.sam.finished";
    &process_cmd($cmd) unless (-e "$target_fa.nameSorted.sam.finished");
    unless ($retain_intermediate_files_flag) {
        &purge_files("$target_fa.sam");
    }
...
sub run_gsnap_alignment_pipeline {
    ...
    my $cmd = "gmap_build -D . -d target.gmap target.fa";
