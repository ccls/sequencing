...
if ($pairs_together_flag && ! ($left_file && $right_file)) {
    ...
    for my $orig_file ( @$source_files ) {
        ...
        while ( my $seq_obj = $reader->next() ) {
            ...
            if ( exists $$idx{$acc} ) {
                ...
                elsif ($seqType eq 'fa') { $record = $seq_obj->get_FASTA_format() }
...
sub run_jellyfish {
    ...
    my $jelly_kmer_fa_file = "jellyfish.$KMER_SIZE.kmers.fa";
...
sub run_nkbc_pairs_together {
    ...
    my $cmd = "$UTILDIR/nbkc_merge_left_right_stats.pl $left_stats_file $right_stats_file > $pair_out_stats_filename";
